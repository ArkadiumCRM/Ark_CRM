# ARK CRM — Account-Detailmaske Interactions Spec v0.2

**Stand:** 13.04.2026
**Status:** Arbeitsdokument — Spec-in-Progress, Review ausstehend
**Kontext:** Definiert Verhalten, Interaktionslogik, CRUD-Flows, Validierung und Speicher-Strategien für alle Tabs der Account-Detailmaske. Gleicher Methodik-Rahmen wie ARK_KANDIDATENMASKE_INTERACTIONS_v1_2.md.
**Begleitdokument:** `ARK_ACCOUNT_DETAILMASKE_SCHEMA_v0_1.md` (Layout, Felder, Design-Tokens)
**Vorrang:** Stammdaten > dieses Dokument > Schema > Mockups
**Globale Patterns:** Es gelten alle 11 globalen Patterns aus TEIL 0 der Kandidaten-Interactions v1.2 (Inline-Edit Hybrid, Tag-CRUD, Dropdowns, Drawer, Speicher-Strategie, Navigation Guard, Optimistic Updates, Toast-Feedback, Confirm-Dialoge, Listen-Strategie, Datepicker).
...
### Snapshot-Bar (zweite Header-Zeile, sticky)
6 KPI-Slots als grosse Zahlen-Kacheln, in jedem Tab sichtbar:
...
| 4 | ⭐ Kulturfit-Score Arkadium Analyse | Berechnet aus Tab 2 Profil & Kultur |
| 5 | 📋 Offene Mandate via Arkadium | `count(fact_mandate WHERE account_id = X AND status = 'Aktiv')` |
| 6 | ⚙ Aktive Prozesse via Arkadium | `count(fact_process_core WHERE account_id = X AND status = 'Open')` |

Slot 4 zeigt "—" solange noch keine Arkadium-Analyse läuft.
...
Name (mit kleinen Icon-Pills daneben für Champion/Blocker/DM/KC) · Position · Decision Level · Telefon · Email · Letzter Kontakt · Status · Aktionen
...
**Vollansicht** (im Kandidat ist nur das Smart-Embed). 8 Sektionen:

1. **Team-Auswahl** (aus Stellenplan)
2. **AI Team-Analyse** (Harmonien, Spannungsfelder, Lücken, Empfehlung — Mit/Ohne Toggle)
...
| Honorar | Erfolgsbasis / Retainer / RPO |
| Erstellt am | Datum |
...
- Toggle "Nach Mandat gruppieren": gruppiert nach `mandate_id`, Mandat-Header → zugehörige Jobs (für Retainer/RPO-Sicht)

---

## TEIL 8: TAB 7 — MANDATE
...
## TEIL 8b: TAB 8 — ASSESSMENTS (NEU v0.2)
...
fact_assessment_order (
  ...
  kandidat_id,           -- immer gesetzt (getestete Person; neu angelegt wenn noch nicht im System)
...
- **Filter-Chips oben:** Alle / Mandatsbezogen / Eigenständig / In Progress / Completed
- **Teamrad-KPI-Banner** prominent oben:
  ```
  📊 Teamrad-Abdeckung: 12 von 47 Mitarbeitenden haben ein Assessment. [→ Teamrad ansehen]
  ```
  Nutzer-Sprung zu Tab 5 Subtab Teamrad.
- **Tabelle** mit allen Assessments (Spalten siehe Schema § 12.2)
...
**Drawer-Schritte:**
1. **Kandidat wählen** (Multi-Select aus Kontakten des Accounts ODER Suche in allen Kandidaten)
   - Falls Kandidat nicht existiert: "→ Als Kandidat anlegen" (Hard-Stop wie in Tab 3 Kontakte)
2. **Mandats-Verknüpfung** (optional):
   - Radio: "Mandatsbezogen" (Dropdown aktive Mandate dieses Accounts) / "Eigenständig"
   - Bei Mandatsbezogen: Auto-Flag als Option IX am verknüpften Mandat
3. **Package-Typ:** Diagnostik / Full / Executive Summary
4. **Preis & Partner:** CHF (case-by-case), Partner-Dropdown (SCHEELEN Default)
5. **Offerte generieren** → PDF aus `Vorlage_Offerte Diagnostik & Assessment.docx`
6. **Upload unterschriebene Offerte** → `status = 'ordered'`, `signed_document_id` gesetzt
7. **Auto-Aktionen:**
   - `fact_history` Eintrag am Account + bei jedem beteiligten Kandidaten
   - Bei Mandatsbezug: `fact_mandate_option` Eintrag mit `option_type = 'IX_assessment'` + `assessment_order_id` FK
   - Rechnungszeile in `fact_assessment_billing` (Default: `billing_type = 'full'`, `due_date = completed_at`)
...
## TEIL 8c: TAB 9 — SCHUTZFRISTEN (NEU v0.2)
...
- Vorstellungs-Typ (email_dossier / verbal_*)
...
1. Drawer mit Honorar-Berechnung:
   - Gehalt-Annahme (AM gibt ein, Default aus Placement-Referenz falls vorhanden)
   - Staffel-Anwendung: [[honorar-berechnung]] Erfolgsbasis-Staffel (21/23/25/27%) oder Mandats-Konditionen
   - Vorschau Rechnungsbetrag
2. "Rechnung erstellen" → generiert Claim-Rechnung
3. `protection_window.status = 'claim_pending'` → `'paid'` nach Zahlung
4. Dokument-Generator: PDF aus Claim-Rechnungs-Template (zu erstellen in Phase 1.5)
5. Event `claim_invoiced` im History des Accounts + des Kandidaten
...
| Aktion | AM (Owner) | AM (andere) | Admin | Backoffice |
|--------|-----------|-------------|---------|-----------|
| Matrix lesen | ✅ | ✅ | ✅ | ⚠ |
| Info-Request senden | ✅ | ❌ | ✅ | ❌ |
| Claim stellen | ⚠ (Freigabe Admin) | ❌ | ✅ | ❌ |
...
| **Auto-Reminders** | Post-Placement (30/60/90d), Briefing-Follow-up | **Mandats-Reminders** (Garantiefrist-Ende, Quartals-Review), **AGB-Erinnerung**, **Scraper-Review** |
...
- Firmengruppe selbst hat eine eigene Detailseite (`/account-groups/[id]`) für Stammdaten-Pflege der Gruppe
...
- Verbleibend: account-spezifische Felder (decision_level, is_*, relationship_score, communication_preference, best_contact_time, do_not_contact, location_id, comment_short, onboarding_notes, last_contacted_*, valid_*)
